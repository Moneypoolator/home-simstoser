...
public:
    ...
    // Загрузка файла
    [[nodiscard]] bool upload_file(const std::string& filename, const std::vector<char>& data);
    ...
    [[nodiscard]] bool set_custom_metadata(const std::string& filename, const std::map<std::string, std::string>& metadata);
    ...
    [[nodiscard]] bool complete_multipart_upload(
        const std::string& upload_id,
        const std::vector<int>& part_numbers
    );
    
    // Отменить загрузку
    [[nodiscard]] bool abort_multipart_upload(const std::string& upload_id);
    ...
    [[nodiscard]] bool write_to_stream(
        const std::string& stream_id,
        const std::vector<char>& data
    );
    
    // Завершить стриминговую загрузку
    [[nodiscard]] bool complete_stream_upload(const std::string& stream_id);
    
    // Отменить стриминговую загрузку
    [[nodiscard]] bool abort_stream_upload(const std::string& stream_id);
    ...
    [[nodiscard]] bool upload_file_stream(const std::string& filename, std::istream& data);
    [[nodiscard]] bool download_file_stream(const std::string& filename, std::ostream& out);
...
private:
   ...
   // Вычисление ETag из данных
    ...
    [[nodiscard]] bool is_path_safe(const std::string& filename) const;
    ...
    [[nodiscard]] bool merge_parts(const multipart_upload& upload, const std::vector<int>& part_numbers);
    ...
    [[nodiscard]] bool save_custom_metadata(const std::string& filename, const std::map<std::string, std::string>& metadata) const;
    ...
    [[nodiscard]] bool copy_with_buffer(std::ifstream& src, std::ofstream& dst, size_t buffer_size = 1024 * 1024);
    [[nodiscard]] bool copy_with_sendfile(int src_fd, int dst_fd, off_t size);
};